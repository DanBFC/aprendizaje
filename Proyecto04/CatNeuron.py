...
# Test path windows
saver_path = r"D:\\Documents\\aprendizaje\\Proyecto04\\Log\\kittyModel"
...
def multilayer_perceptron(x, peso, sesgo):
    ...
    capa_2 = tf.add(tf.matmul(x, pesos['h2'], sesgo['b2']))
    capa_2 = tf.nn.relu(capa_2)

    capa_3 = tf.add(tf.matmul(x, pesos['h3'], sesgo['b3']))
    capa_3 = tf.nn.relu(capa_3)

    capa_4 = tf.add(tf.matmul(x, pesos['h4'], sesgo['b4']))
    capa_4 = tf.nn.relu(capa_4)

    capa_5 = tf.add(tf.matmul(x, pesos['h5'], sesgo['b5']))
    capa_5 = tf.nn.relu(capa_5)

    out_layer = tf.matmul(capa_5, weights['out']) + biases['out']
    return out_layer
...
with tf.Session() as sess:
    ...
    for epoca in range(epocas):
        avg_cost = 0
        lote_total = num_examples / lote
        for i in range(lote_total):
            lote_x, lote_y = ???
            _, c = sess.run([optimizador, Costo, merged_summary_op],
